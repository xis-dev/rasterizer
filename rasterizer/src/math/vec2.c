...
void vec2_construct_mut(vec2* v, float s) { v->e[0] = v->e[1] = s;}
...
float vec2_magnitude(vec2 v1){ return sqrt((v1.e[0] * v1.e[0]) + (v1.e[1] * v1.e[1])); }
...
float vec2_dist_sq(vec2 v1, vec2 v2) {
    ...
    return (d.e[0] * d.e[0]) + (d.e[1] * d.e[1]);
}
