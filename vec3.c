...
void vec3_construct_mut(vec3* v, double s) { v->x = v->y = v->z = s;}
...
double vec3_magnitude(vec3 v1){ return sqrt((v1.x * v1.x) + (v1.y * v1.y) + (v1.z * v1.z)); }
...
void vec3_normalize_mut(vec3* v) {
    ...
    double length = vec3_magnitude(*v);
